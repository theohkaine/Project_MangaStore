...
@WebServlet("/ProductMainPageServlet")
public class ProductMainPageServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
	ProductMainPage products = new ProductMainPage();
	ProductMainPageDAO productsDAO = new ProductMPImplBD();
	ArrayList<ProductMainPage> prod = new ArrayList<ProductMainPage>();
    public ProductMainPageServlet() {
        super();
    }

    //Servlet that adds every corresponding book from the database into the jsp file
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		String pageOutput ="/index.jsp";
		
		try {
			prod = productsDAO.findAll();
